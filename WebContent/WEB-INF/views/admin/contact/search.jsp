...
<div class="row">
    <div class="panel-heading">
        <div class="panel-title ">Quản Lí Liên hệ</div>
    </div>
</div>
<hr>
<div class="row">
    <div class="col-md-4 col-md-push-8">
    	<form action="${pageContext.request.contextPath}/admin/contact/search" method="get">
            	...
            	<c:choose>
            		<c:when test="${not empty listContacts }">
            			<c:forEach items="${listContacts }" var="itemContact">
	            			<c:set var="urlEdit" value="${pageContext.request.contextPath }/admin/contact/edit/${itemContact.id }"></c:set>
	            			<c:set var="urlDel" value="${pageContext.request.contextPath }/admin/contact/del/${itemContact.id }"></c:set>
	            			<c:set var="activeClass" value=""></c:set>
	            			<c:if test="${objContact.id eq itemContact.id }">
			                    <c:set var="activeClass" value="tb-active"></c:set>	
			                </c:if>
	            			<tr class="odd gradeX">
			                    <td class="${activeClass }">${itemContact.id }</td>
			                    <c:set var="styleSearchString" value="<span style='color: red'>${searchString }</span>"></c:set>
			                    <td>${fn:replace(itemContact.fullname, searchString, styleSearchString)}</td>
			                    <td>${itemContact.email }</td>
			                    <td>${itemContact.subject }</td>
			                    <td>${itemContact.content }</td>
            		...
            		<c:otherwise>
            			<tr><td colspan="6"><div class="text-center"><strong>Không tìm thấy <span class="text-danger">${searchString }</span></strong></div></td></tr>
            		</c:otherwise>
            	</c:choose>
            </tbody>
        </table>
    ...
    <nav class="text-center" aria-label="...">
        <ul class="pagination">
        	<c:if test="${numberOfPages > 0 }">
	        	...
	        	<c:if test="${page != 1 }">
	        		 <li class=""><a href="${pageContext.request.contextPath}/admin/contact/search-${searchString }-${page-1}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
	        	</c:if>
	        		...
	        		<c:forEach items="${paginations }" var="itemPagination">
	        			<c:choose>
	        				<c:when test="${itemPagination == page }">
	        					<li class="active"><a href="${pageContext.request.contextPath}/admin/contact/search-${searchString }-${itemPagination}">${itemPagination } <span class="sr-only">(current)</span></a></li>
	        				</c:when>
	        				...
	        				<c:otherwise>
	        					<li><a href="${pageContext.request.contextPath}/admin/contact/search-${searchString }-${itemPagination}">${itemPagination }</a></li>
	        				</c:otherwise>
	        			</c:choose>
	        		</c:forEach>
	        	</c:if>
	        	...
	        	<c:if test="${page != numberOfPages }">
	        		 <li><a href="${pageContext.request.contextPath}/admin/contact/search-${searchString }-${page + 1}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
	        	</c:if>
        	</c:if>
        </ul>
    </nav>
